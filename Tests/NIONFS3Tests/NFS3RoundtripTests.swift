...
final class NFS3RoundtripTests: XCTestCase {
    func testRegularCallsRoundtrip() {
        ...
        let readDirPlusCall1 = NFS3Call.readdirplus(.init(fileHandle: NFS3FileHandle(#line), cookie: 345, cookieVerifier: 879, dirCount: 23488, maxCount: 2342888))
    ...
    func testCallsWithMaxIntegersRoundtrip() {
        let accessCall1 = NFS3Call.access(NFS3CallAccess(object: NFS3FileHandle(.max), access: NFS3Access(rawValue: .max)))
        ...
        let readCall1 = NFS3Call.read(.init(fileHandle: NFS3FileHandle(.max), offset: .max, count: .max))
        let readDirPlusCall1 = NFS3Call.readdirplus(.init(fileHandle: NFS3FileHandle(.max), cookie: .max, cookieVerifier: .max, dirCount: .max, maxCount: .max))
        let readDirCall1 = NFS3Call.readdir(.init(fileHandle: NFS3FileHandle(.max), cookie: .max, cookieVerifier: .max, maxResultByteCount: .max))
    ...
    func testRegularOkayRepliesRoundtrip() {
        func makeRandomFileAttr() -> NFS3FileAttr {
            return .init(type: .init(rawValue: .random(in: 1 ... 7))!,
                         mode: .random(in: 0o000 ... 0o777),
                         nlink: .random(in: .min ... .max),
                         uid: .random(in: .min ... .max),
                         gid: .random(in: .min ... .max),
                         size: .random(in: .min ... .max),
                         used: .random(in: .min ... .max),
                         rdev: .random(in: .min ... .max),
        ...
        let fsInfoReply1 = NFS3Reply.fsinfo(.init(result: .okay(.init(attributes: makeRandomFileAttr(),
        ...
        let fsStatReply1 = NFS3Reply.fsstat(.init(result: .okay(.init(attributes: makeRandomFileAttr(),
                                                                     ...
                                                                     fbytes: .random(in: .min ... .max),
                                                                     abytes: .random(in: .min ... .max),
                                                                     tfiles: .random(in: .min ... .max),
                                                                     ffiles: .random(in: .min ... .max),
                                                                     afiles: .random(in: .min ... .max),
                                                                     invarsec: .random(in: .min ... .max)))))
        let getattrReply1 = NFS3Reply.getattr(.init(result: .okay(.init(attributes: makeRandomFileAttr()))))
        let lookupReply1 = NFS3Reply.lookup(.init(result: .okay(.init(fileHandle: NFS3FileHandle(.random(in: .min ... .max)),
        ...
        let readReply1 = NFS3Reply.read(.init(result: .okay(.init(attributes: makeRandomFileAttr(),
                                                                 count: .random(in: .min ... .max),
                                                                 eof: .random(),
                                                                 data: ByteBuffer(string: "abc")))))
        let readDirPlusReply1 = NFS3Reply.readdirplus(.init(result: .okay(.init(dirAttributes: makeRandomFileAttr(),
                                                                                cookieVerifier: .random(in: .min ... .max),
                                                                                entries: [.init(fileID: .random(in: .min ... .max),
                                                                                                fileName: "asd",
                                                                                                cookie: .random(in: .min ... .max),
                                                                                                nameAttributes: makeRandomFileAttr(),
                                                                                                nameHandle: NFS3FileHandle(.random(in: .min ... .max)))],
                                                                                eof: .random()))))
        let readDirReply1 = NFS3Reply.readdir(.init(result: .okay(.init(dirAttributes: makeRandomFileAttr(),
                                                                        cookieVerifier: .random(in: .min ... .max),
                                                                        entries: [.init(fileID: .random(in: .min ... .max),
                                                                                        fileName: "asd",
                                                                                        cookie: .random(in: .min ... .max))],
        ...
        var xid: UInt32 = 0
        var prepopulatedProcs: [UInt32: RPCNFSProcedureID] = [:]
