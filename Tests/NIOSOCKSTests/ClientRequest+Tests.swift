...
extension ClientRequestTests {
    ...
    func testWriteClientRequest() {
        ...
        let req = ClientRequest(command: .connect, addressType: .init(address: try! .init(ipAddress: "192.168.1.1", port: 80)))
...
extension ClientRequestTests {
    ...
    func testReadAddressType() {
        ...
        XCTAssertNoThrow(XCTAssertEqual(try ipv4.readAddresType(), .init(address: try! .init(ipAddress: "10.11.12.13", port: 80))))
        ...
        XCTAssertNoThrow(XCTAssertEqual(try ipv6.readAddresType(), .init(address: try! .init(ipAddress: "0102:0304:0506:0708:090a:0b0c:0d0e:0fa0", port: 80))))
    ...
    func testWriteAddressType(){
        var ipv4 = ByteBuffer()
        XCTAssertEqual(ipv4.writeAddressType(.init(address: try! .init(ipAddress: "192.168.1.1", port: 80))), 7)
        XCTAssertEqual(ipv4.readBytes(length: 5)!, [1, 192, 168, 1, 1])
        XCTAssertEqual(ipv4.readInteger(as: UInt16.self)!, 80)
        
        var ipv6 = ByteBuffer()
        XCTAssertEqual(ipv6.writeAddressType(.init(address: try! .init(ipAddress: "0001:0002:0003:0004:0005:0006:0007:0008", port: 80))), 19)
